/**
 * Google Sheets Service for Cloudflare Workers
 * Uses Web Crypto API to sign JWT for Service Account authentication
 */
...
export async function readSheetValues(token: string, spreadsheetId: string, range: string) {
...
export async function syncPersonnel(db: D1Database, token: string, spreadsheetId: string) {
